...
{
    data_server_info_t *dsi = &ms.data_server.info[ms.data_server.id];
...
{
    name_server_info_t *nsi = &ms.name_server.info[ms.name_server.id];
...
child_fail_return:
    ...
    if(ss->server_type == SERVER_TYPE_DATA)
        ms.data_server.info[ms.data_server.id].state = SERVER_OFFLINE;
    else
        ms.name_server.info[ms.data_server.id].state = SERVER_OFFLINE;
...
int __data_server_next(ip4_addr_t *addr)
{
    ...
    if(next_id == 0)
    {
        ...
        ms.data_server.start = time(NULL);
    }
    ms.data_server.id = next_id;
    memcpy(addr, &ms.data_server.info[next_id].addr, sizeof(*addr));
...
{
    int next_id = (ms.name_server.id + 1) % ms.name_server.count;
    if(next_id == 0)
    {
        if(ms.name_server.start == time(NULL))
            return MRT_ERR;
        ms.name_server.start = time(NULL);

    }

    ms.name_server.id = next_id;

    memcpy(addr, &ms.name_server.info[ms.name_server.id].addr, sizeof(*addr));
...
loop_next_return:
    ...
    if(timer_event_add(ms.ie, it) == MRT_ERR)
    {
...
int server_check_init()
{
   ...
   // memset(&ms, 0, sizeof(ms));
    ...
    if(!(ms.data_server.info = M_alloc(ds_conf.server_count * sizeof(data_server_info_t))))
    {
        log_error("M_alloc data_server_info error");
        goto error_return;
    }
    ms.data_server.count = ds_conf.server_count;

    if(!(ms.name_server.info = M_alloc(ns_conf.server_count * sizeof(name_server_info_t))))
    {
        log_error("M_alloc data_server_info error");
        goto error_return;
    }
    ms.name_server.count = ns_conf.server_count;

    for(i=0; i<ms.data_server.count; i++)
    {
        memcpy(&ms.data_server.info[i].addr, &ds_conf.server[i].server, sizeof(ds_conf.server[i].server));
    }

    for(i=0; i<ms.name_server.count; i++)
    {
        memcpy(&ms.name_server.info[i].addr, &ns_conf.server[i].master, sizeof(ns_conf.server[i].master));
    }
    ...
    task_init(&ms.data_server.task);
    ms.data_server.task.data = ss_data;

    task_init(&ms.name_server.task);
    ms.name_server.task.data = ss_name;

    if(__server_check_loop(&ms.data_server.task) == MRT_ERR)
    {
    ...
    if(__server_check_loop(&ms.name_server.task) == MRT_ERR)
    {
...
error_return:
    if(ms.data_server.info)
        M_free(ms.data_server.info);

    if(ms.name_server.info)
        M_free(ms.name_server.info);
